...
using namespace std;
class  FE_solver_2D_wave :public FE_solver_2D
{
public:
	//非稳态c(x,y)，f（x,y,t），u（x,y,t)
	FE_solver_2D_wave(double start, double end, double dt, double u00,int N1_, int N2_, int mesh_type_, double a_x, double a_y, double b_x, double b_y, int basis_type_trial_, int basis_type_test_);
	...
	/*注意，迭代的时候A_tilde、A_fixed以及迭代方程和热方程的不一样了*/
	void IterateInTime(double theta);

	//组装b（tm）.这里波方程与热方程也不一样
	void Assemble_b(double tm);
...
调用：
	...
	//计算X0和x1，即t=t0,t=t1时刻各个有限元节点的值
	void Assemble_X_init_X_second();
};
